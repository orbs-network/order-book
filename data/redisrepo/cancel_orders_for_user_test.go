...
func TestRedisRepository_CancelAllOrdersForUser(t *testing.T) {
	...
	t.Run("should remove single unfilled order for user", func(t *testing.T) {
		...
		mock.ExpectZRem(CreateUserOpenOrdersKey(mocks.UserId), mocks.OrderId.String()).SetVal(1)
	...
	t.Run("should remove multiple unfilled orders for user", func(t *testing.T) {
		...
		mock.ExpectTxPipelineExec()
		mock.ExpectTxPipeline()
		mock.ExpectZRem(CreateSellSidePricesKey(mocks.Symbol), orderTwo.Id.String()).SetVal(1)
		mock.ExpectZRem(CreateUserOpenOrdersKey(mocks.UserId), orderTwo.Id.String()).SetVal(1)
