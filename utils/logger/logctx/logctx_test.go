...
func TestFrom(t *testing.T) {
	...
	t.Run("able to differentiate multiple contexts", func(t *testing.T) {
		...
		assert.ElementsMatch(t, []zap.Field{zap.Bool("first_bool", true), zap.String("first_string", "A")}, e1.Context)

		m2 := o2.FilterMessage(secondMessage)
		require.NotZero(t, m2.Len())
		e2 := m2.TakeAll()[0]
		assert.Equal(t, zapcore.InfoLevel, e2.Level)
		assert.ElementsMatch(t, []zap.Field{zap.Bool("second_bool", false), zap.String("second_string", "B")}, e2.Context)
	})
}
