...
func (h *Handler) GetOrdersForUser(w http.ResponseWriter, r *http.Request) {
	...
	logctx.Info(r.Context(), "user trying to get their orders", logger.String("userId", user.Id.String()))

	orders, totalOrders, err := h.svc.GetOrdersForUser(r.Context(), user.Id)

	if err != nil {
		logctx.Error(r.Context(), "error getting orders for user", logger.Error(err), logger.String("userId", user.Id.String()))
